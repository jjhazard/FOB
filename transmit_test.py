...
def collectCalculateTransmit():
    ...
    transmit(original_sets[0][3], original_means[0], original_means[1], original_means[2])

    stop = False
    while not stop:
        copied_sets = collectData()
        batch_lists = getBatchListsFromDataSets(copied_sets)
        print("Copied Signal: ", copied_sets[0][3])
        copied_means = findMeans(batch_names, batch_lists)
        stop = acceptData()

    diffs = []
    ratios = []
    for i in range(6):
        diffs.append(copied_means[i] - original_means[i])
        ratios.append(copied_means[i] / original_means[i])
        print(batch_names[i], "difference:", round(diffs[i], 5)) 
        print(batch_names[i], "percent difference:", round((100*ratios[i])-1, 5) 
